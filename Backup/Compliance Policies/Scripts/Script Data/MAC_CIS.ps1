...
# Logs to: /Library/Logs/IntuneCISCompliance.log
# Output : single-line JSON to stdout | Exit 0
# =============================================================

LOGFILE="/Library/Logs/IntuneCISCompliance.log"
...
if [[ -z "$raw" || "$raw" == "null" ]]; then c_1_6="true"
...
if echo "$raw" | grep -q "true"; then
    ...
    if launchctl list 2>/dev/null | grep -q "com.apple.smbd"; then
        c_2_3_3_2="false"; raw="not listed; smbd loaded"
    else
        c_2_3_3_2="true"; raw="not listed; smbd not loaded"
    fi
fi
...
if [[ -x "$MDATP" ]]; then
    ...
    mde_onboarded="false"; mde_healthy="false"; mde_rtp="false"; mde_nothreats="false"
