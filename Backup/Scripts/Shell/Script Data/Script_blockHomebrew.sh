...
appname="blockHomebrew" # The name of our App deployment script                                           # The actual name of our App once installed
...
if [ -d "$HOMEBREW_PATH" ]; then
    echo "# $(date) | $HOMEBREW_PATH exists. Uninstalling Homebrew and setting permissions."
    # Uninstall Homebrew
    NONINTERACTIVE=1 /bin/bash -c "$(curl -fsSL https://raw.githubusercontent.com/Homebrew/install/HEAD/uninstall.sh)"
    echo "# $(date) | Homebrew uninstalled."
    ...
    echo "# $(date) | $HOMEBREW_PATH does not exist. Creating directory and setting permissions."
...
add_alias_to_profile() {
    profile_path=$1
    echo "# $(date) | Checking $profile_path"

    # Check if the profile file exists and is writable
    if [[ -w "$profile_path" ]]; then
        echo "# $(date) | Adding alias to $profile_path"

        # Check if alias already exists in the profile
        if ! grep -q "alias brew=" "$profile_path"; then
            # Add alias
            echo "# $(date) | alias brew=$alias_message" >>"$profile_path"
        else
            echo "# $(date) | Alias already exists in $profile_path"
        fi
    else
        echo "# $(date) | $profile_path not found or not writable."
    fi
}

# Add the sudoers configuration to restrict chmod on Homebrew directory
add_sudoers_restriction() {
    echo "# $(date) | Attempting to add sudoers restriction for Homebrew directory"
    ...
    if [[ -f "$tmp_sudoers" ]]; then
        ...
        sudo visudo -cf "$tmp_sudoers" && sudo cp "$tmp_sudoers" /etc/sudoers && echo "# $(date) | Sudoers file updated successfully."

        # Clean up the temporary file
        rm "$tmp_sudoers"
    else
        echo "# $(date) | Failed to create temporary sudoers file."
    fi
}
