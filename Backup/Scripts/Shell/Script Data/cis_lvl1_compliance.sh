...
EOS
    ...
    customref="$(echo "$customref" | tr " " ",")"
    if [[ $result_value == "30d" ]]; then
        logmessage "audit_retention_configure passed (Result: $result_value, Expected: \"{'string': '30d'}\")"
        ...
        /usr/bin/logger "mSCP: cis_lvl1 - audit_retention_configure passed (Result: $result_value, Expected: "{'string': '30d'}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            logmessage "audit_retention_configure failed (Result: $result_value, Expected: \"{'string': '30d'}\")"
            /usr/bin/defaults write "$audit_plist" audit_retention_configure -dict-add finding -bool YES
            if [[ ! "$customref" == "audit_retention_configure" ]]; then
                /usr/bin/defaults write "$audit_plist" audit_retention_configure -dict-add reference -string "$customref"
            fi
            /usr/bin/logger "mSCP: cis_lvl1 - audit_retention_configure failed (Result: $result_value, Expected: "{'string': '30d'}")"
        else
            logmessage "audit_retention_configure failed (Result: $result_value, Expected: \"{'string': '30d'}\") - Exemption Allowed (Reason: \"$exempt_reason\")"
            /usr/bin/defaults write "$audit_plist" audit_retention_configure -dict-add finding -bool YES
            if [[ ! "$customref" == "audit_retention_configure" ]]; then
              /usr/bin/defaults write "$audit_plist" audit_retention_configure -dict-add reference -string "$customref"
            fi
            /usr/bin/logger "mSCP: cis_lvl1 - audit_retention_configure failed (Result: $result_value, Expected: "{'string': '30d'}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
if [[ "$arch" == "$rule_arch" ]] || [[ -z "$rule_arch" ]]; then
    unset result_value
    result_value=$(/sbin/mount | /usr/bin/grep ' / ' | /usr/bin/grep -o 'sealed'
)
    # expected result {'string': 'sealed'}
...
EOS
    ...
    customref="$(echo "$customref" | tr " " ",")"
    if [[ $result_value == "sealed" ]]; then
        logmessage "os_authenticated_root_enable passed (Result: $result_value, Expected: \"{'string': 'sealed'}\")"
        ...
        /usr/bin/logger "mSCP: cis_lvl1 - os_authenticated_root_enable passed (Result: $result_value, Expected: "{'string': 'sealed'}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            logmessage "os_authenticated_root_enable failed (Result: $result_value, Expected: \"{'string': 'sealed'}\")"
            /usr/bin/defaults write "$audit_plist" os_authenticated_root_enable -dict-add finding -bool YES
            if [[ ! "$customref" == "os_authenticated_root_enable" ]]; then
                /usr/bin/defaults write "$audit_plist" os_authenticated_root_enable -dict-add reference -string "$customref"
            fi
            /usr/bin/logger "mSCP: cis_lvl1 - os_authenticated_root_enable failed (Result: $result_value, Expected: "{'string': 'sealed'}")"
        else
            logmessage "os_authenticated_root_enable failed (Result: $result_value, Expected: \"{'string': 'sealed'}\") - Exemption Allowed (Reason: \"$exempt_reason\")"
            /usr/bin/defaults write "$audit_plist" os_authenticated_root_enable -dict-add finding -bool YES
            if [[ ! "$customref" == "os_authenticated_root_enable" ]]; then
              /usr/bin/defaults write "$audit_plist" os_authenticated_root_enable -dict-add reference -string "$customref"
            fi
            /usr/bin/logger "mSCP: cis_lvl1 - os_authenticated_root_enable failed (Result: $result_value, Expected: "{'string': 'sealed'}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
#####----- Rule: os_external_apfs_hfs_volumes_encrypted -----#####
## Addresses the following NIST 800-53 controls: 
# * N/A
rule_arch=""
if [[ "$arch" == "$rule_arch" ]] || [[ -z "$rule_arch" ]]; then
    unset result_value
    result_value=$(fail=$(/usr/sbin/diskutil list external | /usr/bin/grep -E "APFS Volume|Apple_HFS|Logical Volume" | /usr/bin/awk '{print $NF}' | /usr/bin/xargs -n1 /usr/sbin/diskutil info 2>/dev/null | /usr/bin/awk '
...
if [ -z "$fail" ]; then
  ...
  /bin/echo "Unencrypted external volumes: $fail"
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cis_lvl1.audit').objectForKey('os_external_apfs_hfs_volumes_encrypted'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cis_lvl1.audit').objectForKey('os_external_apfs_hfs_volumes_encrypted'))["exempt_reason"]
EOS
)   
    customref="$(echo "os_external_apfs_hfs_volumes_encrypted" | rev | cut -d ' ' -f 2- | rev)"
    customref="$(echo "$customref" | tr " " ",")"
    if [[ $result_value == "Yes" ]]; then
        logmessage "os_external_apfs_hfs_volumes_encrypted passed (Result: $result_value, Expected: \"{'string': 'Yes'}\")"
        /usr/bin/defaults write "$audit_plist" os_external_apfs_hfs_volumes_encrypted -dict-add finding -bool NO
        if [[ ! "$customref" == "os_external_apfs_hfs_volumes_encrypted" ]]; then
            /usr/bin/defaults write "$audit_plist" os_external_apfs_hfs_volumes_encrypted -dict-add reference -string "$customref"
        fi
        /usr/bin/logger "mSCP: cis_lvl1 - os_external_apfs_hfs_volumes_encrypted passed (Result: $result_value, Expected: "{'string': 'Yes'}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            logmessage "os_external_apfs_hfs_volumes_encrypted failed (Result: $result_value, Expected: \"{'string': 'Yes'}\")"
            /usr/bin/defaults write "$audit_plist" os_external_apfs_hfs_volumes_encrypted -dict-add finding -bool YES
            if [[ ! "$customref" == "os_external_apfs_hfs_volumes_encrypted" ]]; then
                /usr/bin/defaults write "$audit_plist" os_external_apfs_hfs_volumes_encrypted -dict-add reference -string "$customref"
            fi
            /usr/bin/logger "mSCP: cis_lvl1 - os_external_apfs_hfs_volumes_encrypted failed (Result: $result_value, Expected: "{'string': 'Yes'}")"
        else
            logmessage "os_external_apfs_hfs_volumes_encrypted failed (Result: $result_value, Expected: \"{'string': 'Yes'}\") - Exemption Allowed (Reason: \"$exempt_reason\")"
            /usr/bin/defaults write "$audit_plist" os_external_apfs_hfs_volumes_encrypted -dict-add finding -bool YES
            if [[ ! "$customref" == "os_external_apfs_hfs_volumes_encrypted" ]]; then
              /usr/bin/defaults write "$audit_plist" os_external_apfs_hfs_volumes_encrypted -dict-add reference -string "$customref"
            fi
            /usr/bin/logger "mSCP: cis_lvl1 - os_external_apfs_hfs_volumes_encrypted failed (Result: $result_value, Expected: "{'string': 'Yes'}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
    ...
    logmessage "os_external_apfs_hfs_volumes_encrypted does not apply to this architecture"
    /usr/bin/defaults write "$audit_plist" os_external_apfs_hfs_volumes_encrypted -dict-add finding -bool NO
fi
...
#####----- Rule: os_internal_apfs_volumes_encrypted -----#####
## Addresses the following NIST 800-53 controls: 
# * N/A
rule_arch=""
if [[ "$arch" == "$rule_arch" ]] || [[ -z "$rule_arch" ]]; then
    unset result_value
    result_value=$(fail=$(/usr/sbin/diskutil list internal | /usr/bin/grep "APFS Volume" | /usr/bin/awk '{print $NF}' | /usr/bin/xargs -n1 /usr/sbin/diskutil info 2>/dev/null | /usr/bin/awk '
  /Volume Name:/{name=$0; sub(/^[ \t]*Volume Name:[ \t]*/,"",name)}
  /FileVault:/{
...
if [ -z "$fail" ]; then
  ...
  /bin/echo "Unencrypted internal user APFS volumes: $fail"
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cis_lvl1.audit').objectForKey('os_internal_apfs_volumes_encrypted'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cis_lvl1.audit').objectForKey('os_internal_apfs_volumes_encrypted'))["exempt_reason"]
EOS
)   
    customref="$(echo "os_internal_apfs_volumes_encrypted" | rev | cut -d ' ' -f 2- | rev)"
    customref="$(echo "$customref" | tr " " ",")"
    if [[ $result_value == "Yes" ]]; then
        logmessage "os_internal_apfs_volumes_encrypted passed (Result: $result_value, Expected: \"{'string': 'Yes'}\")"
        /usr/bin/defaults write "$audit_plist" os_internal_apfs_volumes_encrypted -dict-add finding -bool NO
        if [[ ! "$customref" == "os_internal_apfs_volumes_encrypted" ]]; then
            /usr/bin/defaults write "$audit_plist" os_internal_apfs_volumes_encrypted -dict-add reference -string "$customref"
        fi
        /usr/bin/logger "mSCP: cis_lvl1 - os_internal_apfs_volumes_encrypted passed (Result: $result_value, Expected: "{'string': 'Yes'}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            logmessage "os_internal_apfs_volumes_encrypted failed (Result: $result_value, Expected: \"{'string': 'Yes'}\")"
            /usr/bin/defaults write "$audit_plist" os_internal_apfs_volumes_encrypted -dict-add finding -bool YES
            if [[ ! "$customref" == "os_internal_apfs_volumes_encrypted" ]]; then
                /usr/bin/defaults write "$audit_plist" os_internal_apfs_volumes_encrypted -dict-add reference -string "$customref"
            fi
            /usr/bin/logger "mSCP: cis_lvl1 - os_internal_apfs_volumes_encrypted failed (Result: $result_value, Expected: "{'string': 'Yes'}")"
        else
            logmessage "os_internal_apfs_volumes_encrypted failed (Result: $result_value, Expected: \"{'string': 'Yes'}\") - Exemption Allowed (Reason: \"$exempt_reason\")"
            /usr/bin/defaults write "$audit_plist" os_internal_apfs_volumes_encrypted -dict-add finding -bool YES
            if [[ ! "$customref" == "os_internal_apfs_volumes_encrypted" ]]; then
              /usr/bin/defaults write "$audit_plist" os_internal_apfs_volumes_encrypted -dict-add reference -string "$customref"
            fi
            /usr/bin/logger "mSCP: cis_lvl1 - os_internal_apfs_volumes_encrypted failed (Result: $result_value, Expected: "{'string': 'Yes'}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
    ...
    logmessage "os_internal_apfs_volumes_encrypted does not apply to this architecture"
    /usr/bin/defaults write "$audit_plist" os_internal_apfs_volumes_encrypted -dict-add finding -bool NO
fi
...
if [[ "$arch" == "$rule_arch" ]] || [[ -z "$rule_arch" ]]; then
    unset result_value
    result_value=$(/usr/bin/dscl '/Local/Default' read '/Users/root' AuthenticationAuthority 2>/dev/null | /usr/bin/grep -c 'AuthenticationAuthority'
)
    # expected result {'integer': 0}
...
EOS
    ...
    customref="$(echo "$customref" | tr " " ",")"
    if [[ $result_value == "0" ]]; then
        logmessage "os_root_disable passed (Result: $result_value, Expected: \"{'integer': 0}\")"
        ...
        /usr/bin/logger "mSCP: cis_lvl1 - os_root_disable passed (Result: $result_value, Expected: "{'integer': 0}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            logmessage "os_root_disable failed (Result: $result_value, Expected: \"{'integer': 0}\")"
            /usr/bin/defaults write "$audit_plist" os_root_disable -dict-add finding -bool YES
            if [[ ! "$customref" == "os_root_disable" ]]; then
                /usr/bin/defaults write "$audit_plist" os_root_disable -dict-add reference -string "$customref"
            fi
            /usr/bin/logger "mSCP: cis_lvl1 - os_root_disable failed (Result: $result_value, Expected: "{'integer': 0}")"
        else
            logmessage "os_root_disable failed (Result: $result_value, Expected: \"{'integer': 0}\") - Exemption Allowed (Reason: \"$exempt_reason\")"
            /usr/bin/defaults write "$audit_plist" os_root_disable -dict-add finding -bool YES
            if [[ ! "$customref" == "os_root_disable" ]]; then
              /usr/bin/defaults write "$audit_plist" os_root_disable -dict-add reference -string "$customref"
            fi
            /usr/bin/logger "mSCP: cis_lvl1 - os_root_disable failed (Result: $result_value, Expected: "{'integer': 0}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
if [[ "$arch" == "$rule_arch" ]] || [[ -z "$rule_arch" ]]; then
    unset result_value
    result_value=$(/usr/bin/pwpolicy -getaccountpolicies 2> /dev/null | /usr/bin/tail +2 | /usr/bin/xmllint --xpath '//dict/key[text()="policyAttributePasswordHistoryDepth"]/following-sibling::*[1]/text()' - | /usr/bin/awk '{ if ($1 >= 24 ) {print "pass"} else {print "fail"}}' | /usr/bin/uniq
)
    # expected result {'string': 'pass'}
...
if [[ "$arch" == "$rule_arch" ]] || [[ -z "$rule_arch" ]]; then
    unset result_value
    result_value=$(/usr/sbin/sysadminctl -smbGuestAccess status 2>&1 | /usr/bin/grep -c "SMB guest access disabled"
)
    # expected result {'integer': 1}
...
EOS
    ...
    customref="$(echo "$customref" | tr " " ",")"
    if [[ $result_value == "1" ]]; then
        logmessage "system_settings_guest_access_smb_disable passed (Result: $result_value, Expected: \"{'integer': 1}\")"
        ...
        /usr/bin/logger "mSCP: cis_lvl1 - system_settings_guest_access_smb_disable passed (Result: $result_value, Expected: "{'integer': 1}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            logmessage "system_settings_guest_access_smb_disable failed (Result: $result_value, Expected: \"{'integer': 1}\")"
            /usr/bin/defaults write "$audit_plist" system_settings_guest_access_smb_disable -dict-add finding -bool YES
            if [[ ! "$customref" == "system_settings_guest_access_smb_disable" ]]; then
                /usr/bin/defaults write "$audit_plist" system_settings_guest_access_smb_disable -dict-add reference -string "$customref"
            fi
            /usr/bin/logger "mSCP: cis_lvl1 - system_settings_guest_access_smb_disable failed (Result: $result_value, Expected: "{'integer': 1}")"
        else
            logmessage "system_settings_guest_access_smb_disable failed (Result: $result_value, Expected: \"{'integer': 1}\") - Exemption Allowed (Reason: \"$exempt_reason\")"
            /usr/bin/defaults write "$audit_plist" system_settings_guest_access_smb_disable -dict-add finding -bool YES
            if [[ ! "$customref" == "system_settings_guest_access_smb_disable" ]]; then
              /usr/bin/defaults write "$audit_plist" system_settings_guest_access_smb_disable -dict-add reference -string "$customref"
            fi
            /usr/bin/logger "mSCP: cis_lvl1 - system_settings_guest_access_smb_disable failed (Result: $result_value, Expected: "{'integer': 1}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
if [[ "$bl_corner" != "6" ]] && [[ "$tl_corner" != "6" ]] && [[ "$tr_corner" != "6" ]] && [[ "$br_corner" != "6" ]]; then
  echo "0"
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cis_lvl1.audit').objectForKey('system_settings_hot_corners_secure'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cis_lvl1.audit').objectForKey('system_settings_hot_corners_secure'))["exempt_reason"]
EOS
)   
    ...
    if [[ $result_value == "0" ]]; then
        ...
        /usr/bin/logger "mSCP: cis_lvl1 - system_settings_hot_corners_secure passed (Result: $result_value, Expected: "{'integer': 0}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: cis_lvl1 - system_settings_hot_corners_secure failed (Result: $result_value, Expected: "{'integer': 0}")"
        else
            ...
            /usr/bin/logger "mSCP: cis_lvl1 - system_settings_hot_corners_secure failed (Result: $result_value, Expected: "{'integer': 0}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
EOS
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cis_lvl1.audit').objectForKey('system_settings_screensaver_password_enforce'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('org.cis_lvl1.audit').objectForKey('system_settings_screensaver_password_enforce'))["exempt_reason"]
EOS
)   
    ...
    if [[ $result_value == "true" ]]; then
        ...
        /usr/bin/logger "mSCP: cis_lvl1 - system_settings_screensaver_password_enforce passed (Result: $result_value, Expected: "{'string': 'true'}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: cis_lvl1 - system_settings_screensaver_password_enforce failed (Result: $result_value, Expected: "{'string': 'true'}")"
        else
            ...
            /usr/bin/logger "mSCP: cis_lvl1 - system_settings_screensaver_password_enforce failed (Result: $result_value, Expected: "{'string': 'true'}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
    if [[ $audit_retention_configure_audit_score == "true" ]]; then
        ask 'audit_retention_configure - Run the command(s)-> /usr/bin/sed -i.bak '"'"'s/^expire-after.*/expire-after:30d/'"'"' /etc/security/audit_control; /usr/sbin/audit -s ' N
        if [[ $? == 0 ]]; then
            logmessage "Running the command to configure the settings for: audit_retention_configure ..."
            /usr/bin/sed -i.bak 's/^expire-after.*/expire-after:30d/' /etc/security/audit_control; /usr/sbin/audit -s
        fi
    else
...
if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
    if [[ $os_root_disable_audit_score == "true" ]]; then
        ask 'os_root_disable - Run the command(s)-> /usr/bin/fdesetup remove -user root
/usr/bin/dscl '"'"'/Local/Default'"'"' delete '"'"'/Users/root'"'"' AuthenticationAuthority ' N
        if [[ $? == 0 ]]; then
